...
package com.pureMVC.controller.business.common
{
	...
	public class ValidateCommand extends SimpleCommand
	{
			...
			try
			{
				...
				for each (var vVO:ValidateVO in vArr)
				{
					if(vVO.is_Null)
					{
						...
						{
							sendValidateErrorVO(vArr,vVO,Msg.Msg_7,note.getType());
							vArr=null;
							return;
						}
					}
					if(vVO.is_MaxZero)
					{
						...
						{
							sendValidateErrorVO(vArr,vVO,Msg.Msg_21,note.getType());
							vArr=null;
							return;
						}						
					}
					if(vVO.is_Even)
					{
						if(!onEven(int(vVO.text)))
						{
							sendValidateErrorVO(vArr,vVO,Msg.Msg_23,note.getType());
							vArr=null;
							return;							
						}
					}
		 ...
		 * @return 
		 */
		private function onEven(num:int):Boolean
		{
			var bol:Boolean;
			if(num%2==0)
				bol=true;
