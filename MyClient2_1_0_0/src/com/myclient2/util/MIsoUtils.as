...
package com.myclient2.util
{
	...
	/**
	 * MIsoUtils是"等角"地图的工具类,它用来负责"等角"地图坐标换算工作
	 * @author 王明凡
	 */
	public class MIsoUtils
	{
		/**
		 * "等角"地图类型
		 * @default 
		...
		/**
		 * 等角空间中的3D坐标点转换成屏幕上的2D坐标点的校准参数：1.2247的精确计算
		 * @default 
		 */
		 ...
		 * @return 
		 */
		public static function isoToScreen(pt3D:MIsoPoint3D):Point
		{
		 ...
		 * @return 
		 */
		public static function isoToScreen2(x:int,y:int,z:int):Point
		{
			var vertex3D:MIsoPoint3D=new MIsoPoint3D(x, y, z);
			var vertex2D:Point=MIsoUtils.isoToScreen(vertex3D);
			vertex3D=null;
			return vertex2D;
		}		
		/**
		 ...
		 * @return 
		 */
		public static function screenToIso(pt2D:Point):MIsoPoint3D
		{
			var xpos:Number = pt2D.y + (pt2D.x >>1);
			var ypos:Number = 0;
			var zpos:Number = pt2D.y - (pt2D.x >>1);
			pt2D=null;
			return new MIsoPoint3D(xpos, ypos, zpos);
		}
		/**
		 * 获得等角对象的移动3D坐标
		 * @param row
		 ...
		 * @return 
		 */
		public static function getMove3D(row:int,column:int,tileH:int):MIsoPoint3D
		{
			//获得等角矩形
			var isoRect:Rectangle=MIsoUtils.getIsoRect(row,column,tileH);
			//获得右顶点2D坐标
			var right2D:Point=MIsoUtils.isoToScreen2(column * tileH, 0, 0);
			//计算等角矩形显示完全包含等角对象，等角对象需要移动多少3D坐标
			return MIsoUtils.screenToIso(new Point(isoRect.width - right2D.x, tileH >>1));				
		}
		/**
		 * 获得等角对象的等角矩形
		 * @param row
		...
		public static function getIsoRect(row:int,column:int,tileH:int):Rectangle
		{
			...
			var top2D:Point=isoToScreen2(0 * tileH, 0, 0 * tileH);
			var bottom2D:Point=isoToScreen2(column * tileH, 0, row * tileH);
			var left2D:Point=isoToScreen2(0, 0, row * tileH);
			var right2D:Point=isoToScreen2(column * tileH, 0, 0);	
		 ...
		 * @return 
		 */
		public static function getIsometricIndex(pt2D:Point,tileH:int,move3D:MIsoPoint3D):Point
		{
			//转换当前屏幕2D任意坐标到3D坐标
			var pt3D:MIsoPoint3D=MIsoUtils.screenToIso(pt2D);	
			//减去路点被移动的3D坐标
			...
			move3D=null;		
			//获得路点实际的3D坐标
			var Npt3D:MIsoPoint3D=new MIsoPoint3D(mx,my,mz);	
			//转换路点实际的3D坐标成索引					
		 ...
		 * @return 
		 */
		public static function getIsometricPoint(ix:int,iy:int,tileH:int,move3D:MIsoPoint3D):Point
		{
			//加上路点被移动的3D坐标
			var mx:Number=move3D.x+(ix*tileH);
			var my:Number=move3D.y+0;
			var mz:Number=move3D.z+(iy*tileH);
			var pt3D:MIsoPoint3D=new MIsoPoint3D(mx,my,mz);
			move3D=null;	
			return MIsoUtils.isoToScreen(pt3D);				
		}	
	}
}